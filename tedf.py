...
def equacao_mestra(t, psi_vec, R0, lambda_val, alpha_val, N_modos=10):
    """Implementa a equação mestra da TEDF para solve_ivp com proteção numérica.
...
def ajustar_parametros_cmb(dados_observados, ell_range, C_lcdm_base_func):
    ...
    def funcao_custo(params):
        ...
        try:
            C_pred = espectro_tedf(ell_range, lambda_val, alpha_val, C_lcdm=C_lcdm_base_func(ell_range))
...
def salvar_resultados(resultados, diretorio="resultados_tedf"):
    ...
    if 'psi' in resultados and 'termos' in resultados['psi']:
        ...
        plt.xlabel('Termo $n$')
        plt.ylabel('$|\Psi_n|$ (Magnitude do Termo)')
        plt.title('Convergência da Série para o Campo $\Psi$ (TEDF)')
        plt.grid(True)
    ...
    if 'cmb_analysis' in resultados:
        ...
        C_tedf = espectro_tedf(ell, lambda_otimo, alpha_otimo, C_lcdm=C_lcdm)

        plt.figure()
        plt.loglog(ell, C_lcdm, 'b-', label='$\Lambda$CDM Puro')
        plt.loglog(ell, C_tedf, 'r--', label=f'TEDF (lambda={lambda_otimo:.3f}, alpha={alpha_otimo:.3f})' )
        plt.xlabel('Multipolo ($\ell$)')
        plt.ylabel('$C_\ell$ (Espectro de Potência)')
        plt.title('Espectro de Potência Angular do CMB com Correção Fractal')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
    ...
    if 'soliton_params' in resultados:
        ...
        plt.xlabel('Coordenada Espacial $R$')
        plt.ylabel('$|\Psi(R, t)|$ (Amplitude do Soliton)')
        plt.title("Solitons da TEDF (alpha=3/2)" ) # Alpha fixo para esta solução
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
    ...
    if 'evolucao_temporal' in resultados:
        ...
        plt.xlabel('Tempo $t$')
        plt.ylabel('Valor do Campo $\Psi(t)$')
        plt.title('Evolução Temporal do Campo $\Psi$ (TEDF)')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
    ...
    if 'varredura_parametros' in resultados and len(resultados['varredura_parametros']) > 0:
        ...
        if not df.empty:
            ...
            if not df_filtrado.empty:
                ...
                plt.xlabel('$\lambda$ (Acoplamento Fractal)')
                plt.ylabel('Magnitude de $|\Psi|$')
                plt.title('Magnitude de $\Psi$ na Varredura de Parâmetros')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
...
def executar_simulacoes_completas(diretorio_saida="resultados_tedf_precisos"):
    """Executa todas as simulações e análises da TEDF, salvando os resultados.
    ...
    """
    resultados = {}
    print(f"Iniciando simulações completas da TEDF. Resultados serão salvos em: {diretorio_saida}")
    ...
    print("\nSimulações completas da TEDF concluídas com sucesso!")
    return resultados
...
if __name__ == "__main__":
    ...
    # Define o diretório de saída para os resultados (Downloads/Resultados_TEDF)
    output_dir = os.path.join(os.path.expanduser('~'), 'Downloads', 'Resultados_TEDF')
